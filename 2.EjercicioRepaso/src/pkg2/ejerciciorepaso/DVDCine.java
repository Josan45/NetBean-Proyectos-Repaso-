/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package pkg2.ejerciciorepaso;

import java.util.Scanner;

/**
 *
 * @author Alumno_2DAW
 */
public class DVDCine {

    /**
     * @param args the command line arguments
     */
    public static void main(String[] args) {
        // TODO code application logic here
        System.out.println("Pelicula");
        System.out.println("introduzca los datos de una pelicula");
        Scanner leer=new Scanner(System.in);
        String titulo = null,
                actor = null, 
                director = null, 
                genero = null,
                duracion = null,
                resumen = null;
        System.out.println("Introduzca el titulo");
        titulo=leer.nextLine();
        System.out.println("Introduzca el actor");
        actor=leer.nextLine();
        System.out.println("Introduzca el director");
        director=leer.nextLine();
        System.out.println("Introduzca el genero");
        genero=leer.nextLine();
        System.out.println("Introduzca el duracion");
        duracion=leer.nextLine();
        System.out.println("Introduzca el resumen");
        resumen=leer.nextLine();
        DVD pelicula=new DVD(titulo, actor, director, genero, duracion, resumen);
        System.out.println(pelicula.muestraDVD());
        
        
    }

    
}
